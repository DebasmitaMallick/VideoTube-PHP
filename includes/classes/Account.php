...
class Account {
    ...
    public function register($fn, $ln, $un, $role, $number, $em, $em2, $pw, $pw2) {
        ...
        if(empty($this->errorArray)) {
            return $this->insertUserDetails($fn, $ln, $un, $role, $number, $em, $pw);
        }
        else {
            return false;
        }
    ...
    public function insertUserDetails($fn, $ln, $un, $role, $number, $em, $pw) {
        
        $pw = hash("sha512", $pw);
        $profilePic = "assets/images/profilePictures/default.png";

        $query = $this->con->prepare("INSERT INTO users (firstName, lastName, username, role, number, email, password, profilePic)
                                        VALUES(:fn, :ln, :un, :role, :number, :em, :pw, :pic)");
    ...
    private function validateRole($role) {
        if($role != 'normalUser') {
            array_push($this->errorArray, Constants::$roleInvalid);
        }
    }

    private function validateNumber($number) {
        if(strlen($number) != 10 || !is_numeric($number)) {
            array_push($this->errorArray, Constants::$phoneNo);
        }
    }
