<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(isset($_POST['superAdmin'])){
            $sql = "INSERT INTO privileges(userId, role) VALUES(:userId, :role);
                    UPDATE `users` SET `role` = 'superAdmin' WHERE `users`.`id` = :userId;";
    ...
    // CHANGED::E
    $query = $con->query("SELECT * FROM privileges WHERE privileges.role = 'superAdmin'");
    while ($row = $query->fetch(PDO::FETCH_ASSOC)){
        if(isset($_POST['removeRoleAdmin'.$row['userId']]) or isset($_POST['super'.$row['userId']])){
            $deleteFlag = true;
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        //             <td><input onclick="checkError(this, `disabledCheckbox'.$x.'`)" class="categoryCheckbox" type="checkbox" id="'.$x.'" name="'.$x.'" value="'.$x.'"><label for="'.$x.'">'.$item.'</label></td>
        //             <td><input class="disabledCheckbox'.$x.' disabled2" type="checkbox" name="acessUpload'.$x.'" value="acessUpload'.$x.'" ><label for="acessUpload'.$x.'">Upload</label></td>
        //             <td><input class="disabledCheckbox'.$x.' disabled2" type="checkbox" name="acessDelete'.$x.'" value="acessDelete'.$x.'" ><label for="acessDelete'.$x.'"> Delete</label></td>
        //         </tr>';
        $out .= '
                <tr>
                    <td><input type="checkbox" id="'.$x.'" name="'.$x.'" value="'.$x.'"><label for="'.$x.'">'.$item.'</label></td>
                    <td><input type="checkbox" name="acessUpload'.$x.'" value="acessUpload'.$x.'"><label for="acessUpload'.$x.'">Upload</label></td>
                    <td><input type="checkbox" name="acessDelete'.$x.'" value="acessDelete'.$x.'"><label for="acessDelete'.$x.'"> Delete</label></td>
                </tr>';
        // CHANGED FINAL::E
    }
    ...
    while($row = $query->fetch(PDO::FETCH_ASSOC)) {
        ...
        '<div class="card mb-2 mr-4 assignRolesCards">
            ...
            <div class="card-body">
                ...
                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#editModal'.$row['id'].'">View</button>
...
<div class="modal fade" id="myModal">
    <div class="modal-dialog">
        <form id="roleForm" class="modal-content" action="assignRoles.php" method="post">
            ...
            <div class="modal-body" style="text-align: left !important;">
                <div class="pb-2">
                    <input type="checkbox" id="super" name="superAdmin" value="super">
                    <label for="super">SUPER superAdmin</label>
                </div>
